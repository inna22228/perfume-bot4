...
# === НАСТРОЙКИ ===
BOT_TOKEN = "8506330687:AAGTTez5ZdFbNdVZxuOAq6J316chnOJDr-M"
GROUP_CHAT_ID = -1003462107945  # Твоя приватная группа
...
async def show_volumes(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.answer()
    perfume_index = int(query.data.split("_")[1])
    p = PERFUMES[perfume_index]
    keyboard = [
        [InlineKeyboardButton(f"{vol} мл — {price} ₽", callback_data=f"add_{perfume_index}_{vol}")]
...
def main():
    ...
    print("✅ Бот с корзиной и телефоном запущен!")
    app.run_polling()
